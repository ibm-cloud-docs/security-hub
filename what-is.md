...
### Accelerate security with Terraform IBM Modules
{: #tim-intro}

One of the most effective ways to eliminate misconfiguration is to define your security infrastructure as code — so that every environment is provisioned identically, every change is reviewed, and every configuration is auditable. {{site.data.keyword.cloud_notm}} provides a library of open-source [Terraform IBM Modules (TIM)](https://cloud.ibm.com/docs/ibm-cloud-provider-for-terraform?topic=ibm-cloud-provider-for-terraform-about-tim){: external} — production-ready, best-practice-aligned Terraform configurations for core security services. Rather than configuring each service manually, you can compose these modules to build a consistent, repeatable security foundation across all your accounts and environments.

Throughout this guide, you will find references to the relevant TIM for each service discussed. You can browse the full module library on the [Terraform Registry](https://registry.terraform.io/namespaces/terraform-ibm-modules){: external} or on the [Terraform IBM Modules GitHub organization](https://github.com/terraform-ibm-modules){: external}.
